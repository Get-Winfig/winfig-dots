...
# ---------------------------------------------------------------------------- #
#  Powertoys Setup Function
function SetupPowertoys {
    try {
        $TemplatesSrc = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Powertoys", "Templates")
        $TemplatesDst = "C:\\Windows\\Resources\\Templates"
        $BackupDst = Join-Path $env:UserProfile "Documents\\PowerToys\\Backup"
        if (-not (Test-Path $BackupDst)) {
        ...
        if (Test-Path $TemplatesDst) {
            try {
                Remove-Item $TemplatesDst -Force -Recurse
                Log-Message -Message "Removed existing $TemplatesDst" -Level "INFO"
            } catch {
                Show-WarningMessage "Could not remove $TemplatesDst. It may be in use. Skipping link."
                Log-Message -Message ("Could not remove $TemplatesDst") -Level "WARN"
            }
        }
        try {
            New-Item -ItemType SymbolicLink -Path $TemplatesDst -Target $TemplatesSrc -Force | Out-Null
            Show-SuccessMessage "Symlinked Templates to $TemplatesDst"
            Log-Message -Message "Symlinked $TemplatesSrc to $TemplatesDst" -Level "SUCCESS"
        } catch {
            Show-WarningMessage "Could not create symlink for Templates: $($_.Exception.Message)"
            Log-Message -Message "Could not create symlink for Templates: $($_.Exception.Message)" -Level "WARN"
        }

        # Symlink all settings_134123738397103391.ptb files to Backup folder
        $settingsFiles = Get-ChildItem -Path ([System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Powertoys","settings_134123738397103391.ptb")) -File
        foreach ($file in $settingsFiles) {
            $dst = Join-Path $BackupDst $file.Name
            if (Test-Path $dst) {
                try {
                    ...
                    Log-Message -Message ("Could not remove $dst") -Level "WARN"
                    continue
                }
            }
            try {
                New-Item -ItemType SymbolicLink -Path $dst -Target $file.FullName -Force | Out-Null
                Show-SuccessMessage "Symlinked $($file.Name) to $dst"
                Log-Message -Message "Symlinked $($file.FullName) to $dst" -Level "SUCCESS"
            } catch {
                Show-WarningMessage "Could not create symlink for $($file.Name): $($_.Exception.Message)"
                Log-Message -Message "Could not create symlink for $($file.Name): $($_.Exception.Message)" -Level "WARN"
            }
        }

        try {
            # Import settings (this will open PowerToys)
            Start-Process -FilePath "PowerToys.exe" -ArgumentList @("--import-settings", $latestBackup.FullName) -Wait
            Show-SuccessMessage "Loaded PowerToys settings from $($latestBackup.FullName)"
            Log-Message -Message "Loaded PowerToys settings from $($latestBackup.FullName)" -Level "SUCCESS"

            # Print user instructions for manual restore
            Write-Host "" -ForegroundColor $Script:WinfigColors.Info
            Write-Host "[ACTION REQUIRED]" -ForegroundColor $Script:WinfigColors.Accent
            Write-Host "1. PowerToys should now be open." -ForegroundColor $Script:WinfigColors.Primary
            Write-Host "2. Go to the 'General' tab." -ForegroundColor $Script:WinfigColors.Primary
            Write-Host "3. Scroll down to 'Backup & Restore'." -ForegroundColor $Script:WinfigColors.Primary
            Write-Host "4. Click the 'Restore' button to apply your settings." -ForegroundColor $Script:WinfigColors.Primary
            Write-Host "5. PowerToys will restart automatically after restore." -ForegroundColor $Script:WinfigColors.Primary
            Write-Host "" -ForegroundColor $Script:WinfigColors.Info
            Show-InfoMessage "Follow the above steps in PowerToys to complete the restore."
            Log-Message -Message "Displayed manual restore instructions for PowerToys settings." -Level "INFO"
        } catch {
            Show-WarningMessage "Could not load PowerToys settings or display instructions: $($_.Exception.Message)"
            Log-Message -Message "Could not load PowerToys settings or display instructions: $($_.Exception.Message)" -Level "WARN"
        }
    } catch {
        Show-ErrorMessage "Failed to setup Powertoys configuration: $($_.Exception.Message)"
        Log-Message -Message "Failed to setup Powertoys configuration: $($_.Exception.Message)" -Level "ERROR"
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Setup Powertoys Configuration"
