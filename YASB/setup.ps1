...
# ---------------------------------------------------------------------------- #
# Clone Winfig Wallpapers Repo if not exists
function Clone-Wallpapers {
    $path = "C:\Windows\Web\Wallpaper\winfig-wallpaper"
    $url = "https://github.com/Get-winfig/winfig-wallpaper.git"

    if (-not (Test-Path -Path $path)) {
        try {
            Show-InfoMessage "Cloning Winfig Wallpapers repository..."
            Show-WarningMessage "It may take some time depending on internet connection..."
            Log-Message -Message "Cloning Winfig Wallpapers repository..." -Level "INFO"
            git clone $url $path *> $null
        } catch {
            Show-ErrorMessage "Failed to clone Winfig Wallpapers repository: $($_.Exception.Message)"
            Log-Message -Message "Failed to clone Winfig Wallpapers repository: $($_.Exception.Message)" -Level "ERROR"
            exit 1
        }
        if (Test-Path -Path $path) {
            Show-SuccessMessage "Cloned Winfig Wallpapers repository to $path."
            Log-Message -Message "Cloned Winfig Wallpapers repository to $path." -Level "SUCCESS"
        } else {
            Show-ErrorMessage "Winfig Wallpapers repository was not cloned. Please check your internet connection or repository URL."
            Log-Message -Message "Winfig Wallpapers repository was not cloned. Please check your internet connection or repository URL." -Level "ERROR"
            exit 1
        }
    } else {
        try {
            Show-InfoMessage "Updating Winfig Wallpapers repository..."
            Log-Message -Message "Updating Winfig Wallpapers repository..." -Level "INFO"
            Push-Location $path
            git pull *> $null
            Pop-Location
            Show-SuccessMessage "Updated Winfig Wallpapers repository at $path."
            Log-Message -Message "Updated Winfig Wallpapers repository at $path." -Level "SUCCESS"
        } catch {
            Show-ErrorMessage "Failed to update Winfig Wallpapers repository: $($_.Exception.Message)"
            Log-Message -Message "Failed to update Winfig Wallpapers repository: $($_.Exception.Message)" -Level "ERROR"
            exit 1
        }
    }
}
...
function SetupYASB {
    try {
        $YASB    = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "YASB", "config.yaml")
        $styles  = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "YASB", "styles.css")
        $target  = [System.IO.Path]::Combine($Global:WinfigPaths.UserProfile, ".config", "yasb")
        ...
        Show-ErrorMessage "Failed to setup YASB configuration: $($_.Exception.Message)"
        Log-Message -Message "Failed to setup YASB configuration: $($_.Exception.Message)" -Level "ERROR"
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Cloning Winfig Wallpapers Repository"
...
Winfig-Banner
Write-SectionHeader -Title "Setup YASB Configuration"

SetupYASB
yasbc enable-autostart *> $null
Log-Message -Message "YASB autostart enabled"
yasbc stop *> $null
yasbc start *> $null
Log-Message -Message "Started YASB"
komorebic stop --whkd *> $null
Log-Message -Message "Stopped Komorebic"
komorebic start --whkd *> $null
Log-Message -Message "Started Komorebic"
