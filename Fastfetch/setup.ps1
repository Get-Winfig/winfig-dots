...
# ---------------------------------------------------------------------------- #
#  FastFetch Setup Function
function SetupFastfetch {
    try {
        $Config    = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Fastfetch", "config.jsonc")
        $Logo   = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Fastfetch", "winfig.txt")
        $target =[System.IO.Path]::Combine($env:UserProfile, ".config", "fastfetch")
        ...
        #  Find "%Logo%" in config.jsonc file and replace it with "$target\winfig.txt"
        $configContent = Get-Content -Path $Config -Raw
        $logoPath = Join-Path $target "winfig.txt"
        $updatedContent = $configContent -replace "%Logo%", $logoPath.Replace('\', '\\')
        # Save the updated content back to config.jsonc
        Set-Content -Path $Config -Value $updatedContent
        ...
        Show-ErrorMessage "Failed to setup Fastfetch configuration: $($_.Exception.Message)"
        Log-Message -Message "Failed to setup Fastfetch configuration: $($_.Exception.Message)" -Level "ERROR"
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Setup Fastfetch Configuration"
