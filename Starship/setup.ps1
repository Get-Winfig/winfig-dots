...
# ---------------------------------------------------------------------------- #
#  Starship Setup Function
function SetupStarship {
    try {
        $Config    = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Starship", "starship.toml")
        $target     = [System.IO.Path]::Combine($env:UserProfile, ".config")
        ...
        Show-ErrorMessage "Failed to setup Starship configuration: $($_.Exception.Message)"
        Log-Message -Message "Failed to setup Starship configuration: $($_.Exception.Message)" -Level "ERROR"
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Setup Starship Configuration"
