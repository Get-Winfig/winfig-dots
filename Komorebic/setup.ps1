...
# ---------------------------------------------------------------------------- #
# Setup System/User Enviromental Variables
function Setup-EnvironmentVariables {
    ...
    $env:KOMOREBI_CONFIG_HOME = $Target
    $env:WHKD_CONFIG_HOME = $Target

    # Set for user environment variables
    [Environment]::SetEnvironmentVariable("KOMOREBI_CONFIG_HOME", $Target, "User")
    [Environment]::SetEnvironmentVariable("WHKD_CONFIG_HOME", $Target, "User")

    # Set for system environment variables (requires admin)
    try {
        [Environment]::SetEnvironmentVariable("KOMOREBI_CONFIG_HOME", $Target, "Machine")
        [Environment]::SetEnvironmentVariable("WHKD_CONFIG_HOME", $Target, "Machine")
        Show-SuccessMessage "Set KOMOREBI_CONFIG_HOME and WHKD_CONFIG_HOME for user and system."
    } catch {
        Show-WarningMessage "Could not set system environment variables (admin required): $($_.Exception.Message)"
    }
}

# ---------------------------------------------------------------------------- #
#  Komorebic Setup Function
function SetupKomorebic {
    try {
        $Komorebic    = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Komorebic", "komorebi.json")
        $applications = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Komorebic", "applications.json")
        $binds        = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Komorebic", "whkdrc")
        $target       = [System.IO.Path]::Combine($Global:WinfigPaths.UserProfile, ".config", "komorebic")

        Setup-EnvironmentVariables -Target $target

        $files = @{
            "komorebi.json" = $Komorebic
        ...
        Show-ErrorMessage "Failed to setup Komorebic configuration: $($_.Exception.Message)"
        Log-Message -Message "Failed to setup Komorebic configuration: $($_.Exception.Message)" -Level "ERROR"
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Setup Komorebic Configuration"

SetupKomorebic
komorebic enable-autostart *> $null
Log-Message -Message "Komorebic autostart enabled"
