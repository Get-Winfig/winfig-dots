...
# ====================================================================== #
$Script:ModulesList = @(
    @{ packageName = "posh-git"; displayName = "posh-git"; Description = "Provides Git status summary information and tab completion in PowerShell." },
    @{ packageName = "Terminal-Icons"; displayName = "Terminal-Icons"; Description = "Adds file and folder icons to your PowerShell terminal for improved readability." },
    @{ packageName = "PowercolorLs"; displayName = "PowercolorLs"; Description = "A colorful and fast replacement for the 'ls' command in PowerShell." },
    @{ packageName = "PSCompletions"; displayName = "PSCompletions"; Description = "Enables advanced tab completion for PowerShell commands and modules." },
    @{ packageName = "Microsoft.PowerShell.Archive"; displayName = "Microsoft.PowerShell.Archive"; Description = "Provides cmdlets for creating and extracting ZIP archives." },
    @{ packageName = "PSWeb"; displayName = "PSWeb"; Description = "A set of web-related utilities for PowerShell, including HTTP requests." },
    @{ packageName = "PSFzf"; displayName = "PSFzf"; Description = "Integrates fzf, a command-line fuzzy finder, into PowerShell for interactive searching." },
    @{ packageName = "PSReadLine"; displayName = "PSReadLine"; Description = "Enhances the PowerShell command-line editing experience with syntax highlighting and history." },
    @{ packageName = "syntax-highlighting"; displayName = "syntax-highlighting"; Description = "Adds syntax highlighting to PowerShell scripts and command-line input." }
)

# Only add WinGet.CommandNotFound if PowerShell 7+
if ($PSVersionTable.PSVersion -ge [Version]"7.0") {
    $Script:ModulesList += @{ packageName = "Microsoft.WinGet.CommandNotFound"; displayName = "Microsoft.WinGet.CommandNotFound"; Description = "Suggests WinGet packages when you enter an unrecognized command in PowerShell 7+." }
}
...
function Install-Modules {
    ...
    foreach ($mod in $Script:ModulesList) {
        $modName    = $mod.packageName
        $modDisplay = $mod.displayName
        $modDesc    = $mod.Description

        Write-SubsectionHeader -Title "Processing: $modDisplay"
        Show-InfoMessage "$modDesc"
        Log-Message -Message "Processing module: $modName ($modDisplay)" -Level "INFO"
        $totalModulesCount++

        # Check if module is already installed
        $isInstalled = Get-InstalledModule -Name $modName -ErrorAction SilentlyContinue
        if ($isInstalled) {
            Show-InfoMessage "Module '$modDisplay' is already installed. Skipping."
            Log-Message -Message "Module '$modName' ($modDisplay) is already installed. Skipping." -Level "INFO"
            $skippedModulesCount++
            Write-Host ""
            continue
        }
        ...
        try {
            ...
            Show-SuccessMessage "Successfully installed '$modDisplay'."
            Log-Message -Message "Successfully installed '$modName' ($modDisplay)." -Level "SUCCESS"
            $installedModulesCount++
        } catch {
            Show-ErrorMessage "Failed to install '$modDisplay': $($_.Exception.Message)"
            Log-Message -Message "Failed to install '$modName' ($modDisplay): $($_.Exception.Message)" -Level "ERROR"
            $failedModulesCount++
        }
        Write-Host ""
    }
    ...
    # Summary
    Write-SubsectionHeader -Title "Module Installation Summary"
    Show-InfoMessage "Total modules: $totalModulesCount"
    Show-InfoMessage "Installed: $installedModulesCount"
    Show-InfoMessage "Skipped (already installed): $skippedModulesCount"
    if ($failedModulesCount -gt 0) {
        Show-ErrorMessage "Failed: $failedModulesCount"
    }
    Log-Message -Message "Module installation completed. Total: $totalModulesCount, Installed: $installedModulesCount, Already Installed: $skippedModulesCount, Failed: $failedModulesCount" -Level "INFO"
}
...
function SetupPowershell {
    try {
        $Config     = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Powershell", "Config")
        $Functions  = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Powershell", "Functions")
        $Aliases    = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Powershell", "Aliases.ps1")
        $Initialize = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Powershell", "Initialize.ps1")
        $Profile    = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Powershell", "Microsoft.PowerShell_profile.ps1")
        $targets = @([System.IO.Path]::Combine($Global:WinfigPaths.Documents, "WindowsPowerShell"))

        # Check if PowerShell 7+ is installed (pwsh.exe in PATH or Program Files)
        $pwshInstalled = Get-Command pwsh -ErrorAction SilentlyContinue
        if ($pwshInstalled) {
            $targets += [System.IO.Path]::Combine($Global:WinfigPaths.Documents, "PowerShell")
        }
        ...
        Show-ErrorMessage "Failed to setup Powershell configuration: $($_.Exception.Message)"
        Log-Message -Message "Failed to setup Powershell configuration: $($_.Exception.Message)" -Level "ERROR"
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Setup Powershell Modules"
...
Winfig-Banner
Write-SectionHeader -Title "Setup Powershell Configuration"
