...
# ---------------------------------------------------------------------------- #
#  Lazygit Setup Function
function SetupLazygit {
    try {
        $Config    = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Lazygit", "config.yml")
        $target     = [System.IO.Path]::Combine($env:UserProfile, ".config", "lazygit")
        ...
        Show-ErrorMessage "Failed to setup Lazygit configuration: $($_.Exception.Message)"
        Log-Message -Message "Failed to setup Lazygit configuration: $($_.Exception.Message)" -Level "ERROR"
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Setup Lazygit Configuration"
