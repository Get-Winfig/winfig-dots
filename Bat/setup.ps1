...
function SetupBat {
    try {
        $Config    = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Bat", "config")
        $Themes   = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Bat", "themes")
        $target =[System.IO.Path]::Combine($Global:WinfigPaths.AppDataRoaming, "bat")
        ...
        if (-not (Test-Path $target)) {
            ...
            Log-Message -Message "Created Bat config directory at $target." -Level "SUCCESS"
        }
        ...
        foreach ($name in $files.Keys) {
            ...
            try {
                ...
                Log-Message -Message "Rebuilt Bat cache after linking $name" -Level "INFO"
        ...
        Show-ErrorMessage "Failed to setup Bat configuration: $($_.Exception.Message)"
        Log-Message -Message "Failed to setup Bat configuration: $($_.Exception.Message)" -Level "ERROR"
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Setup Bat Configuration"
