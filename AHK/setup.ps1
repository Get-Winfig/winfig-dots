...
function SetupAHK {
    try {
        $ahk    = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "AHK", "Hotkeys.ahk")
        $target    =[System.IO.Path]::Combine($Global:WinfigPaths.AppDataRoaming, "Microsoft", "Windows", "Start Menu", "Programs", "Startup")
        ...
        Show-ErrorMessage "Failed to setup AHK configuration: $($_.Exception.Message)"
        Log-Message -Message "Failed to setup AHK configuration: $($_.Exception.Message)" -Level "ERROR"
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Setup AHK Configuration"
